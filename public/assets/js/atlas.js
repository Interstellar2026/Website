...
(function() {
  ...
  function ensureLiveChatModal() {
    ...
    if (!modal) {
      ...
      modal.id = 'livechat-modal';
      modal.className = 'livechat-modal';
      modal.setAttribute('aria-hidden', 'true');
      modal.setAttribute('role', 'dialog');
      modal.setAttribute('aria-label', 'Live chat with Interstellar I.S. team');
      modal.innerHTML =
        '<div class="livechat-header">' +
          '<button class="livechat-back" id="livechat-back" aria-label="Back to Atlas">← Atlas</button>' +
          '<span class="livechat-title">Live ▪ Interstellar I.S.</span>' +
          '<button class="livechat-close" id="livechat-close" aria-label="Close live chat">×</button>' +
        '</div>' +
        '<div class="livechat-loading" id="livechat-loading">' +
  ...
  function start() {
    ...
    state.step = 'greeted';
    botSay("Hey — I'm Atlas. Interstellar I.S.'s AI guide. I can help you figure out what you actually need (and what you're probably overpaying for). Tell me what's on your mind, or pick one below.", function() {
      setOptions([
        { label: 'Cut my bill', action: function() { handleQuickStart('bill_too_high'); } },
        { label: 'Voice / contact center', action: function() { handleQuickStart('voice'); } },
        { label: 'Network & SD-WAN', action: function() { handleQuickStart('network'); } },
        { label: 'Cybersecurity', action: function() { handleQuickStart('security'); } },
        { label: 'Talk to a human', action: function() { routeToTeam(); } }
      ]);
    }, 400);
  }
  ...
  function askSize() {
    ...
    botSay("Roughly how many people work there?", function() {
      setOptions([
        { label: '1–10', action: function(l) { handleSize('1–10', l); } },
        { label: '11–50', action: function(l) { handleSize('11–50', l); } },
        { label: '51–250', action: function(l) { handleSize('51–250', l); } },
        { label: '251–1000', action: function(l) { handleSize('251–1000', l); } },
        { label: '1000+', action: function(l) { handleSize('1000+', l); } }
      ]);
    });
  }
  ...
  function askPain() {
    ...
    botSay("What's the main thing? (Pick the closest.)", function() {
      setOptions([
        { label: 'Bill is too high', action: function(l) { handlePain('bill_too_high', l); } },
        { label: 'Service is unreliable', action: function(l) { handlePain('unreliable', l); } },
        { label: 'Need new tech', action: function(l) { handlePain('new_tech', l); } },
        { label: 'Contract expiring', action: function(l) { handlePain('contract_expiring', l); } },
        { label: 'Just exploring', action: function(l) { handlePain('exploring', l); } }
      ]);
    });
  }
  ...
  function askUrgency() {
    ...
    botSay("When do you need this fixed?", function() {
      setOptions([
        { label: 'This week', action: function(l) { handleUrgency('this_week', l); } },
        { label: 'This month', action: function(l) { handleUrgency('this_month', l); } },
        { label: 'This quarter', action: function(l) { handleUrgency('this_quarter', l); } },
        { label: 'No rush', action: function(l) { handleUrgency('no_rush', l); } }
      ]);
    });
  }
  ...
  function deliverRecommendation() {
    ...
    botSay(summary, function() {
      const opts = [
        { label: 'Schedule it ↗', action: function() { openBookings(); } },
        { label: 'Talk live now', action: function() { routeToTeam(); } },
        { label: 'Just send me info', action: function() { askEmail(); } }
      ];
      if (isHighValue) opts.unshift({ label: 'Talk to the founder', action: function() { routeToFounder(); } });
      setOptions(opts);
    }, 800);
  }
  function openBookings() {
    ...
    clearOptions();
    botSay("Opening MS Bookings in a new tab. Pick a time that works — we'll see you there.", function() {
      window.open(CONFIG.bookingsUrl, '_blank', 'noopener');
      setTimeout(function() {
        botSay("Anything else I can answer in the meantime?", function() {
          setOptions([
            { label: 'How does the call work?', action: function() { answerFAQ('how assessment works'); } },
            { label: 'Is it really free?', action: function() { answerFAQ('cost me'); } },
            { label: 'No, thanks', action: function() { closeFlow(); } }
          ]);
        }, 800);
      }, 1500);
    });
  ...
  function handleEmailCapture(email) {
    ...
    botSay("Got it — sending to " + email + ". You'll hear from us within one business day. Anything else I can help with?", function() {
      setOptions([
        { label: 'How do you make money?', action: function() { answerFAQ('cost me'); } },
        { label: 'How long is the process?', action: function() { answerFAQ('how long process'); } },
        { label: 'No, thanks', action: function() { closeFlow(); } }
      ]);
    });
  }
  function answerFAQ(key) {
    ...
    if (answer) {
      ...
      clearOptions();
      botSay(answer, function() {
        setOptions([
          { label: 'Schedule a call ↗', action: function() { openBookings(); } },
          { label: 'Talk live now', action: function() { routeToTeam(); } },
          { label: 'Other questions', action: function() { offerMoreQuestions(); } }
        ]);
      });
    }
  }
  function offerMoreQuestions() {
    ...
    clearOptions();
    botSay("Sure — what else?", function() {
      setOptions([
        { label: 'What does I.S. mean?', action: function() { answerFAQ('what is i.s.'); } },
        { label: 'Are you really free?', action: function() { answerFAQ('do you charge fees'); } },
        { label: 'Where are you based?', action: function() { answerFAQ('where located'); } },
        { label: "I'm good — talk to team", action: function() { routeToTeam(); } }
      ]);
    });
  }
  function closeFlow() {
  ...
  function handleFreeText(text) {
    ...
    if (state.step === 'collecting_email') {
      ...
      if (emailMatch) handleEmailCapture(emailMatch[0]);
      else botSay("Hmm — that doesn't look like a valid email. Mind trying again?");
      return;
    }
    ...
    botSay("Good question — that's outside what I can answer reliably. Let me get you to a human who can.", function() {
      setOptions([
        { label: 'Connect to team', action: function() { routeToTeam(); } },
        { label: 'Schedule instead', action: function() { openBookings(); } },
        { label: 'Send me info via email', action: function() { askEmail(); } }
      ]);
    });
  }
  function openPanel() {
